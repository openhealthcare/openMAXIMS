...
public abstract class BaseLogic extends Handlers
{
	...
	{
		return ims.pathways.domain.Worklist.class;
	}
	public final void setContext(ims.framework.UIEngine engine, GenForm form, ims.pathways.domain.Worklist domain)
	{
		setContext(engine, form);
		this.domain = domain;
		if(domain.getLookupService().countLookupInstances(ims.pathways.vo.lookups.BreachManagementCollection.class, ims.pathways.vo.lookups.BreachManagement.class, 1321010, true) == 0)
		{
			if(lookupUsageErrors == null)
				lookupUsageErrors = new java.util.ArrayList();
			lookupUsageErrors.add("The lookup type 'BreachManagement' (ID: 1321010) must have at least one active instance configured.");
		}
	}
	...
	protected final void bindgrdTargetsColBreachStatusLookup()
	{
		this.form.grdTargets().ColBreachStatusComboBox().clear();
		ims.pathways.vo.lookups.BreachManagementCollection lookupCollection = ims.pathways.vo.lookups.LookupHelper.getBreachManagement(this.domain.getLookupService());
		for(int x = 0; x < lookupCollection.size(); x++)
		{
			this.form.grdTargets().ColBreachStatusComboBox().newRow(lookupCollection.get(x), lookupCollection.get(x).getText(), lookupCollection.get(x).getImage(), lookupCollection.get(x).getTextColor());
		}
	}
	...
	protected ims.pathways.domain.Worklist domain;
}
