...
public abstract class BaseLogic extends Handlers
{
	...
	{
		return ims.pathways.domain.EventSearchAndSelect.class;
	}
	public final void setContext(ims.framework.UIEngine engine, GenForm form, ims.pathways.domain.EventSearchAndSelect domain)
	{
	...
	protected final void bindgrdEventsColEventTypeLookup()
	{
		this.form.grdEvents().ColEventTypeComboBox().clear();
		...
		{
			this.form.grdEvents().ColEventTypeComboBox().newRow(lookupCollection.get(x), lookupCollection.get(x).getText(), lookupCollection.get(x).getImage(), lookupCollection.get(x).getTextColor());
		}
	}
	protected final void bindgrdEventsColStatusLookup()
	{
		this.form.grdEvents().ColStatusComboBox().clear();
		...
		for(int x = 0; x < lookupCollection.size(); x++)
		{
			this.form.grdEvents().ColStatusComboBox().newRow(lookupCollection.get(x), lookupCollection.get(x).getText(), lookupCollection.get(x).getImage(), lookupCollection.get(x).getTextColor());
		}
	}
	...
	protected ims.pathways.domain.EventSearchAndSelect domain;
}
