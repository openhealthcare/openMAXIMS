...
public abstract class BaseLogic extends Handlers
{
	...
	{
		return ims.icp.domain.ICPStageSelectDialog.class;
	}
	public final void setContext(ims.framework.UIEngine engine, GenForm form, ims.icp.domain.ICPStageSelectDialog domain)
	{
	...
	protected final void bindgrdStagesColPhasesLookup()
	{
		this.form.grdStages().ColPhasesComboBox().clear();
		...
		for(int x = 0; x < lookupCollection.size(); x++)
		{
			this.form.grdStages().ColPhasesComboBox().newRow(lookupCollection.get(x), lookupCollection.get(x).getText(), lookupCollection.get(x).getImage(), lookupCollection.get(x).getTextColor());
		}
	}
	protected final void bindgrdStagesColStatusLookup()
	{
		this.form.grdStages().ColStatusComboBox().clear();
		ims.core.vo.lookups.PreActiveActiveInactiveStatusCollection lookupCollection = ims.core.vo.lookups.LookupHelper.getPreActiveActiveInactiveStatus(this.domain.getLookupService());
		for(int x = 0; x < lookupCollection.size(); x++)
		{
			this.form.grdStages().ColStatusComboBox().newRow(lookupCollection.get(x), lookupCollection.get(x).getText(), lookupCollection.get(x).getImage(), lookupCollection.get(x).getTextColor());
		}
	}
	...
	protected ims.icp.domain.ICPStageSelectDialog domain;
}
