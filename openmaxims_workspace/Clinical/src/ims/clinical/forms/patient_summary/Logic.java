...
import ims.clinical.forms.patient_summary.GenForm.ctnSummaryContainer.GroupProcedureEnumeration;
import ims.clinical.forms.patient_summary.GenForm.ctnSummaryContainer.grdDiagnosisRow;
import ims.clinical.forms.patient_summary.GenForm.ctnSummaryContainer.grdProcedureRow;
...
public class Logic extends BaseLogic
{
	...
	private static final String	COLUMN_STARTDATE_ENDDATE     		= "0";
	private static final String	COLUMN_SPECIALTY					= "1";
	private static final String	COLUMN_CONTACT_TYPE					= "2";
	private static final String	COLUMN_RMO      					= "3";
	private static final String	COLUMN_HISTORY                      = "4";
	...
	private void listCareSpells()
	{
		...
		CareSpellVoCollection voCollCareSpells = domain.listCareSpellsByPatientAndCriteria(form.getGlobalContext().Core.getPatientShort(), form.dteDateFrom().getValue(), form.dteDateTo().getValue(), nContext, form.getLocalContext().getShowCancelledContexts()); // wdev-13335
																																																																	 // ,
																																																																	 // //wdev-13977
		// CareSpellVoCollection voCollCareSpells =
		// domain.listCareSpellsByPatient(form.getGlobalContext().Core.getPatientShort());
	...
	private DynamicGridRow[] getAllRows(DynamicGridRow parentRow)
	{
		java.util.ArrayList rows = new java.util.ArrayList();
		
		for(int x = 0; x < parentRow.getRows().size(); x++)
		{
			DynamicGridRow row = parentRow.getRows().get(x);
			rows.add(row);
		...
		DynamicGridRow[] result = new DynamicGridRow[rows.size()];
		
		for(int x = 0; x < rows.size(); x++)
		{
			result[x] = (DynamicGridRow)rows.get(x);
		}
	...
	{
		DynamicGridRow careSpellRow;
		for (int i = 0; i < voCollCareSpells.size(); i++)
		{
			careSpellRow = form.dyngrdList().getRows().newRow();
			...
			//cellCareSpell.setColumnHistoryEmpty(true); // wdev-13896
			// wdev-13977
			if (voCollCareSpells.get(i).getCurrentStatusIsNotNull() && voCollCareSpells.get(i).getCurrentStatus().getStatusIsNotNull() && voCollCareSpells.get(i).getCurrentStatus().getStatus().equals(ProblemGroupStatus.CANCELLED))
	...
	private void setStartDateEndDateAndTooltip(DynamicGridRow careSpellRow, CareSpellVo careSpellVo)
	{
		DynamicGridCell cellCareSpell = careSpellRow.getCells().newCell(getColumnById(COLUMN_STARTDATE_ENDDATE), DynamicCellType.STRING);
		
		CSDiagnosisAndProcedureTextAndTooltip textAndTooltip = getPrimaryDiagnosisAndProblems(careSpellVo);
		cellCareSpell.setValue(careSpellVo.createDisplayString() + textAndTooltip.getText());
		cellCareSpell.setReadOnly(true);
		cellCareSpell.setTooltip("Problem Group: " + careSpellVo.createDisplayString() + textAndTooltip.getText());
	}

	private boolean populateEpisodeOfCareRows(EpisodeofCareVoCollection voCollEpisodeOfCare, DynamicGridRow careSpellRow)
	{
		int endedCount = 0;
		if (voCollEpisodeOfCare != null && careSpellRow != null)
		{
			DynamicGridRow episodeOfCareRow;
			for (int i = 0; i < voCollEpisodeOfCare.size(); i++)
			{
				episodeOfCareRow = careSpellRow.getRows().newRow();
				
				DynamicGridCell cellStartEndDate = episodeOfCareRow.getCells().newCell(getColumnById(COLUMN_STARTDATE_ENDDATE), DynamicCellType.STRING);
				cellStartEndDate.setValue(voCollEpisodeOfCare.get(i).createDisplayString());
				cellStartEndDate.setTooltip("Episode of Care: " + voCollEpisodeOfCare.get(i).createDisplayString());
				cellStartEndDate.setReadOnly(true);
				
				DynamicGridCell cellSpecialty = episodeOfCareRow.getCells().newCell(getColumnById(COLUMN_SPECIALTY), DynamicCellType.STRING);
				cellSpecialty.setValue(voCollEpisodeOfCare.get(i).getSpecialtyIsNotNull() ? INDENT + voCollEpisodeOfCare.get(i).getSpecialty().getText() : null);
				cellSpecialty.setTooltip("Episode of Care: " + (voCollEpisodeOfCare.get(i).getSpecialtyIsNotNull() ? INDENT + voCollEpisodeOfCare.get(i).getSpecialty().getText() : ""));
				cellSpecialty.setReadOnly(true);
				
				if (voCollEpisodeOfCare.get(i).getResponsibleHCPIsNotNull())
				{
					DynamicGridCell cellRMO = episodeOfCareRow.getCells().newCell(getColumnById(COLUMN_RMO), DynamicCellType.STRING);
					cellRMO.setValue(voCollEpisodeOfCare.get(i).getResponsibleHCP().getMosIsNotNull() ? INDENT + getFirst21Char(voCollEpisodeOfCare.get(i).getResponsibleHCP().getMos().toString()) : null); //wdev-16293
					cellRMO.setTooltip("Episode of Care: " + (voCollEpisodeOfCare.get(i).getResponsibleHCP().getMosIsNotNull() ? INDENT + voCollEpisodeOfCare.get(i).getResponsibleHCP().getMos().toString() : ""));
				...
				// wdev-13977
				if (voCollEpisodeOfCare.get(i).getCurrentStatusIsNotNull() && voCollEpisodeOfCare.get(i).getCurrentStatus().getStatusIsNotNull() && voCollEpisodeOfCare.get(i).getCurrentStatus().getStatus().equals(EpisodeOfCareStatus.CANCELLED))
	...
	private void populateCareContextRows(CareContextVoCollection voCollCareContexts, DynamicGridRow episodeOfCareRow)
	{
		if (voCollCareContexts != null && episodeOfCareRow != null)
		{
			voCollCareContexts.sort();
			DynamicGridRow careContextRow;
			for (int i = 0; i < voCollCareContexts.size(); i++)
			{
				...
				DynamicGridCell cellStartEndDate = careContextRow.getCells().newCell(getColumnById(COLUMN_STARTDATE_ENDDATE), DynamicCellType.STRING);
				cellStartEndDate.setValue(displayString);
				cellStartEndDate.setTooltip("Care Context: " + displayString);
				cellStartEndDate.setReadOnly(true);
				
				DynamicGridCell cellSpecialty = careContextRow.getCells().newCell(getColumnById(COLUMN_SPECIALTY), DynamicCellType.STRING);
				cellSpecialty.setValue(voCollCareContexts.get(i).getContextIsNotNull() ? INDENT + INDENT + voCollCareContexts.get(i).getContext().getText() : null);
				cellSpecialty.setTooltip("Care Context: " + (voCollCareContexts.get(i).getContextIsNotNull() ? INDENT + INDENT + voCollCareContexts.get(i).getContext().getText() : ""));
				cellSpecialty.setReadOnly(true);
				...
				{
					DynamicGridCell cellRMO = careContextRow.getCells().newCell(getColumnById(COLUMN_RMO), DynamicCellType.STRING);
					cellRMO.setValue(voCollCareContexts.get(i).getResponsibleHCP().getIHcpName());
					cellRMO.setReadOnly(true);
				}

				// WDEV-13764
				if (voCollCareContexts.get(i).getCurrentStatusIsNotNull() && voCollCareContexts.get(i).getCurrentStatus().getStatusIsNotNull() && voCollCareContexts.get(i).getCurrentStatus().getStatus().equals(CareContextStatus.CANCELLED))
	...
	private void gridListSelectionChanged()
	{
		...
		if (form.dyngrdList().getValue() instanceof CareSpellVo)
		{
			CareSpellVo careSpell = (CareSpellVo) form.dyngrdList().getValue();
			...
			// wdev-13977
			form.getContextMenus().getClinicalContactsListCancelProblemGroupItem().setVisible(checkIfCareSpellCanBeCancelled((CareSpellVo) form.dyngrdList().getValue()));
			// ---------
			form.getGlobalContext().Clinical.PatientSummary.setCareSpell((CareSpellVo) form.dyngrdList().getValue());
		}
		else if (form.dyngrdList().getValue() instanceof EpisodeofCareVo)
		{
			EpisodeofCareVo episodeOfCare = (EpisodeofCareVo) form.dyngrdList().getValue();
			...
			EpisodeofCareVo voEpisodeofCare = (EpisodeofCareVo) form.dyngrdList().getValue();
			...
			// wdev-13977
			form.getContextMenus().getClinicalContactsListCancelEpisodeOfCareItem().setVisible(checkIfEpisodeOfCareCanBeCancelled((EpisodeofCareVo) form.dyngrdList().getValue()));
			// -----
		}
		else if (form.dyngrdList().getValue() instanceof CareContextVo)
		{
			CareContextVo careContext = (CareContextVo) form.dyngrdList().getValue();
			...
			CareContextVo voCareContext = (CareContextVo) form.dyngrdList().getValue();
			...
			{
				if (form.dyngrdList().getSelectedRow().getParent().getValue() instanceof EpisodeofCareVo)
				{
					EpisodeofCareVo voEpisodeofCare = (EpisodeofCareVo) form.dyngrdList().getSelectedRow().getParent().getValue();
					// form.getGlobalContext().Core.setEpisodeofCareShort(voEpisodeofCare);
			...
			// WDEV-13764
			form.getContextMenus().getClinicalContactsListCancelCareContextItem().setVisible(checkIfCareContextCanBeCancelled((CareContextVo) form.dyngrdList().getValue())); // wdev-13977

			form.ctnSummary().setCaption("Care Context");
		}
		else if (form.dyngrdList().getValue() instanceof ClinicalContactShortVo)
		{
			ClinicalContactShortVo clinicalContact = (ClinicalContactShortVo) form.dyngrdList().getValue();
			...
			form.getContextMenus().getClinicalContactsListViewClinicalNotesItem().setVisible(clinicalContact.getIsClinicalNoteCreatedIsNotNull() && clinicalContact.getIsClinicalNoteCreated().booleanValue() &&
			// WDEV-2511
			...
			if (form.dyngrdList().getSelectedRow().getParent() != null)
			{
				if (form.dyngrdList().getSelectedRow().getParent().getValue() instanceof CareContextVo)
				{
					CareContextVo voCareContext = (CareContextVo) form.dyngrdList().getSelectedRow().getParent().getValue();
					form.getGlobalContext().Core.setCurrentCareContext(voCareContext);

					if (voCareContext.getContextIsNotNull())
						form.getGlobalContext().Core.CurrentCareContext.setContextType(new Integer(voCareContext.getContext().getId()));

					if (form.dyngrdList().getSelectedRow().getParent() != null && form.dyngrdList().getSelectedRow().getParent().getParent() != null)
					{
						if (form.dyngrdList().getSelectedRow().getParent().getParent().getValue() instanceof EpisodeofCareVo)
						{
							EpisodeofCareVo voEpisodeofCare = (EpisodeofCareVo) form.dyngrdList().getSelectedRow().getParent().getParent().getValue();
							// form.getGlobalContext().Core.setEpisodeofCareShort(voEpisodeofCare);
							form.getGlobalContext().Core.setEpisodeofCareShort(domain.getEpisodeOfCareShort(voEpisodeofCare)); // wdev-13335
						}
					}
			...
			// WDEV-13764
			form.getContextMenus().getClinicalContactsListCancelClinicalContactItem().setVisible(clinicalContact.getCurrentStatus() == null || (ClinicalContactStatus.OPEN.equals(clinicalContact.getCurrentStatus().getStatus())));

			form.ctnSummary().setCaption("Clinical Contact");
		}

		setPatientSummaryGlobalContexts(form.dyngrdList().getValue());
	...
	{
		form.btnNewICP().setVisible(Boolean.TRUE.equals(ConfigFlag.UI.ALLOW_PATIENT_ICP_CREATION_FROM_PATIENT_SUMMARY.getValue()) && form.getGlobalContext().Core.getCurrentCareContextIsNotNull());
		// TODO - Add one more condition to check if the selected CareContext
		// has an ICP instantiated
	...
	// only check this according to a config flag and override anyway if the
	// approle right
	...
	private void setPatientSummaryGlobalContexts(Object value)
	{
		...
		{
			// form.getGlobalContext().Clinical.PatientSummary.setCareSpell((CareSpellVo)
			// value);
		...
		if (value instanceof EpisodeofCareVo)
		{
			...
			{
				// form.getGlobalContext().Clinical.PatientSummary.setCareSpell((CareSpellVo)
				// form.grdList().getSelectedRow().getParentRow().getValue());
				form.getGlobalContext().Clinical.PatientSummary.setCareSpell(domain.getCareSpellById((CareSpellVo) form.dyngrdList().getSelectedRow().getParent().getValue())); // wdev-13335
			}
		}
		// Care Context
		if (value instanceof CareContextVo)
		{
			...
			if (form.dyngrdList().getSelectedRow().getParent() != null)
			{
				form.getGlobalContext().Clinical.PatientSummary.setEpisodeOfCare((EpisodeofCareVo) form.dyngrdList().getSelectedRow().getParent().getValue());

				if (form.dyngrdList().getSelectedRow().getParent().getParent() != null)
				{
					// form.getGlobalContext().Clinical.PatientSummary.setCareSpell((CareSpellVo)
					// form.grdList().getSelectedRow().getParentRow().getParentRow().getValue());
					form.getGlobalContext().Clinical.PatientSummary.setCareSpell(domain.getCareSpellById((CareSpellVo) form.dyngrdList().getSelectedRow().getParent().getParent().getValue())); // wdev-13335
				}
			}
		}
		// Clinical Contact
		if (value instanceof ClinicalContactShortVo)
		{
			...
			if (form.dyngrdList().getSelectedRow().getParent() != null)
			{
				form.getGlobalContext().Clinical.PatientSummary.setCareContext((CareContextVo) form.dyngrdList().getSelectedRow().getParent().getValue());
				if (form.dyngrdList().getSelectedRow().getParent().getParent() != null)
				{
					form.getGlobalContext().Clinical.PatientSummary.setEpisodeOfCare((EpisodeofCareVo) form.dyngrdList().getSelectedRow().getParent().getParent().getValue());

					if (form.dyngrdList().getSelectedRow().getParent().getParent().getParent() != null)
					{
						// form.getGlobalContext().Clinical.PatientSummary.setCareSpell((CareSpellVo)
						// form.grdList().getSelectedRow().getParentRow().getParentRow().getParentRow().getValue());
						form.getGlobalContext().Clinical.PatientSummary.setCareSpell(domain.getCareSpellById((CareSpellVo) form.dyngrdList().getSelectedRow().getParent().getParent().getParent().getValue())); // wdev-13335
					}
				}
			}
		}
	...
	private void listCDIDetails(boolean refresh)
	{
		...
		Object value = form.dyngrdList().getValue();
	...
	protected void onContextMenuItemClick(int menuItemID, ims.framework.Control sender) throws ims.framework.exceptions.PresentationLogicException
	{
		...
		if (sender.equals(form.dyngrdList()))
		{
			switch (menuItemID)
			{
	...
	private void undoCancelProblemGroup()
	{
		try
		{
			...
			if (careSpell.getStatusHistory() == null)
	...
	protected void cancelCareSpell()
	{
		if (form.dyngrdList().getValue() instanceof CareSpellVo)
		{
			CareSpellVo tempVo = (CareSpellVo) form.dyngrdList().getValue();
			...
			if (tempVo != null && tempVo.getVersion_CareSpell() != ((CareSpellVo) form.dyngrdList().getValue()).getVersion_CareSpell())
			{
	...
	private void undoCancelEpisodeOfCare()
	{
		try
		{
			...
			if (episodeOfCare.getStatusHistory() == null)
	...
	protected void cancelEpisodeOfCare()
	{
		if (form.dyngrdList().getValue() instanceof EpisodeofCareVo)
		{
			EpisodeofCareVo trempEpOfCare = (EpisodeofCareVo) form.dyngrdList().getValue();
			...
			if (trempEpOfCare != null && trempEpOfCare.getVersion_EpisodeOfCare() != ((EpisodeofCareVo) form.dyngrdList().getValue()).getVersion_EpisodeOfCare())
			{
				engine.showMessage(ims.configuration.gen.ConfigFlag.UI.STALE_OBJECT_MESSAGE.getValue());
				open();
				return;
	...
	{
		if (form.dyngrdList().getValue() instanceof CareContextVo)
		{
			CareContextVo voCareContext = (CareContextVo) form.dyngrdList().getValue();
			...
			if (voCareContext != null && voCareContext.getVersion_CareContext() != ((CareContextVo) form.dyngrdList().getValue()).getVersion_CareContext())
			{
				engine.showMessage(ims.configuration.gen.ConfigFlag.UI.STALE_OBJECT_MESSAGE.getValue());
				open();
				return;
	...
	private void undoCancelCareContext()
	{
		try
		{
			...
			if (careContext.getStatusHistory() == null)
	...
	private void cancelClinicalContact()
	{
		...
		if (form.dyngrdList().getValue() instanceof ClinicalContactShortVo)
		{
			...
			ClinicalContactShortVo voClinicalContact = (ClinicalContactShortVo) form.dyngrdList().getValue();
	...
	protected void onFormDialogClosed(FormName formName, DialogResult result) throws PresentationLogicException
	{
		...
		else if (formName.equals(form.getForms().Therapies.SOAPNoteDialog) || formName.equals(form.getForms().Clinical.ClinicalNoteDrawingDialog) || formName.equals(form.getForms().Clinical.ClinicalNotesDialog))
		{
			// form.getGlobalContext().Core.setClinicalNotesShort(null);
			// form.getGlobalContext().Clinical.setCurrentClinicalNote(null);
		...
		else if (formName.equals(form.getForms().Clinical.DiagnosisComplications_Dialog) || formName.equals(form.getForms().Clinical.PatientProblems_Dialog))
		{
			...
			// WDEV-2439
			DynamicGridRow parentCareSpellRow = getParentCareSpellRow(form.dyngrdList().getSelectedRow());
	...
	// Get the parent CareSpellRow recursively to cover any extra level
	// introduced (if ever)
	private DynamicGridRow getParentCareSpellRow(DynamicGridRow selectedRow)
	{
		if (selectedRow != null)
		{
			...
			return getParentCareSpellRow(selectedRow.getParent());
		}
	...
	// ---------------------------------------------------------
	// If a Previously selected episode is the same as the current one, do not
	// list Diagnosis and Procedures again
	...
	protected void onBtnSearchWorkflowIDClick() throws PresentationLogicException
	{
		...
		DynamicGridRow[] rows = getAllRows(form.dyngrdList().getRows());
	...
	{
		form.dyngrdList().getRows().clear(); // wdev-13624
	...
	@Override
	protected void onDyngrdListRowSelectionChanged(DynamicGridRow row) throws PresentationLogicException
	{
		gridListSelectionChanged();
		updateControlsState();
	}
	...
	@Override
	protected void onDyngrdListRowSelectionCleared() throws PresentationLogicException
	{
		clearSelection();
		enableDiagnosisGridContextMenu(); // WDEV-15918
		enableProcedureContextMenu();
