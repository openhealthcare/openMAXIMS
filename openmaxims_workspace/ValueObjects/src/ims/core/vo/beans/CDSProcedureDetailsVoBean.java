//#############################################################################
//#                                                                           #
//#  Copyright (C) <2014>  <IMS MAXIMS>                                       #
//#                                                                           #
//#  This program is free software: you can redistribute it and/or modify     #
//#  it under the terms of the GNU Affero General Public License as           #
//#  published by the Free Software Foundation, either version 3 of the       #
//#  License, or (at your option) any later version.                          # 
//#                                                                           #
//#  This program is distributed in the hope that it will be useful,          #
//#  but WITHOUT ANY WARRANTY; without even the implied warranty of           #
//#  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the            #
//#  GNU Affero General Public License for more details.                      #
//#                                                                           #
//#  You should have received a copy of the GNU Affero General Public License #
//#  along with this program.  If not, see <http://www.gnu.org/licenses/>.    #
//#                                                                           #
//#############################################################################
//#EOH
// This code was generated by Barbara Worwood using IMS Development Environment (version 1.80 build 5007.25751)
// Copyright (C) 1995-2014 IMS MAXIMS. All rights reserved.
// WARNING: DO NOT MODIFY the content of this file

package ims.core.vo.beans;

public class CDSProcedureDetailsVoBean extends ims.vo.ValueObjectBean
{
	public CDSProcedureDetailsVoBean()
	{
	}
	public CDSProcedureDetailsVoBean(ims.core.vo.CDSProcedureDetailsVo vo)
	{
		this.id = vo.getBoId();
		this.version = vo.getBoVersion();
		this.procedurescheme = vo.getProcedureScheme();
		this.procedure = vo.getProcedure();
		this.proceduredate = vo.getProcedureDate() == null ? null : (ims.framework.utils.beans.DateBean)vo.getProcedureDate().getBean();
		this.laterality = vo.getLaterality();
	}

	public void populate(ims.vo.ValueObjectBeanMap map, ims.core.vo.CDSProcedureDetailsVo vo)
	{
		this.id = vo.getBoId();
		this.version = vo.getBoVersion();
		this.procedurescheme = vo.getProcedureScheme();
		this.procedure = vo.getProcedure();
		this.proceduredate = vo.getProcedureDate() == null ? null : (ims.framework.utils.beans.DateBean)vo.getProcedureDate().getBean();
		this.laterality = vo.getLaterality();
	}

	public ims.core.vo.CDSProcedureDetailsVo buildVo()
	{
		return this.buildVo(new ims.vo.ValueObjectBeanMap());
	}

	public ims.core.vo.CDSProcedureDetailsVo buildVo(ims.vo.ValueObjectBeanMap map)
	{
		ims.core.vo.CDSProcedureDetailsVo vo = null;
		if(map != null)
			vo = (ims.core.vo.CDSProcedureDetailsVo)map.getValueObject(this);
		if(vo == null)
		{
			vo = new ims.core.vo.CDSProcedureDetailsVo();
			map.addValueObject(this, vo);
			vo.populate(map, this);
		}
		return vo;
	}

	public Integer getId()
	{
		return this.id;
	}
	public void setId(Integer value)
	{
		this.id = value;
	}
	public int getVersion()
	{
		return this.version;
	}
	public void setVersion(int value)
	{
		this.version = value;
	}
	public String getProcedureScheme()
	{
		return this.procedurescheme;
	}
	public void setProcedureScheme(String value)
	{
		this.procedurescheme = value;
	}
	public String getProcedure()
	{
		return this.procedure;
	}
	public void setProcedure(String value)
	{
		this.procedure = value;
	}
	public ims.framework.utils.beans.DateBean getProcedureDate()
	{
		return this.proceduredate;
	}
	public void setProcedureDate(ims.framework.utils.beans.DateBean value)
	{
		this.proceduredate = value;
	}
	public String getLaterality()
	{
		return this.laterality;
	}
	public void setLaterality(String value)
	{
		this.laterality = value;
	}

	private Integer id;
	private int version;
	private String procedurescheme;
	private String procedure;
	private ims.framework.utils.beans.DateBean proceduredate;
	private String laterality;
}
