//#############################################################################
//#                                                                           #
//#  Copyright (C) <2014>  <IMS MAXIMS>                                       #
//#                                                                           #
//#  This program is free software: you can redistribute it and/or modify     #
//#  it under the terms of the GNU Affero General Public License as           #
//#  published by the Free Software Foundation, either version 3 of the       #
//#  License, or (at your option) any later version.                          # 
//#                                                                           #
//#  This program is distributed in the hope that it will be useful,          #
//#  but WITHOUT ANY WARRANTY; without even the implied warranty of           #
//#  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the            #
//#  GNU Affero General Public License for more details.                      #
//#                                                                           #
//#  You should have received a copy of the GNU Affero General Public License #
//#  along with this program.  If not, see <http://www.gnu.org/licenses/>.    #
//#                                                                           #
//#############################################################################
//#EOH
// This code was generated by Barbara Worwood using IMS Development Environment (version 1.80 build 5007.25751)
// Copyright (C) 1995-2014 IMS MAXIMS. All rights reserved.
// WARNING: DO NOT MODIFY the content of this file

package ims.nursing.vo.beans;

public class BladderManagementBean extends ims.vo.ValueObjectBean
{
	public BladderManagementBean()
	{
	}
	public BladderManagementBean(ims.nursing.vo.BladderManagement vo)
	{
		this.id = vo.getBoId();
		this.version = vo.getBoVersion();
		this.method = vo.getMethod() == null ? null : (ims.vo.LookupInstanceBean)vo.getMethod().getBean();
		this.type = vo.getType() == null ? null : (ims.vo.LookupInstanceBean)vo.getType().getBean();
		this.size = vo.getSize();
		this.dateinserted = vo.getDateInserted() == null ? null : (ims.framework.utils.beans.DateBean)vo.getDateInserted().getBean();
		this.datenextchange = vo.getDateNextChange() == null ? null : (ims.framework.utils.beans.DateBean)vo.getDateNextChange().getBean();
		this.iscurrentlyactive = vo.getIsCurrentlyActive();
	}

	public void populate(ims.vo.ValueObjectBeanMap map, ims.nursing.vo.BladderManagement vo)
	{
		this.id = vo.getBoId();
		this.version = vo.getBoVersion();
		this.method = vo.getMethod() == null ? null : (ims.vo.LookupInstanceBean)vo.getMethod().getBean();
		this.type = vo.getType() == null ? null : (ims.vo.LookupInstanceBean)vo.getType().getBean();
		this.size = vo.getSize();
		this.dateinserted = vo.getDateInserted() == null ? null : (ims.framework.utils.beans.DateBean)vo.getDateInserted().getBean();
		this.datenextchange = vo.getDateNextChange() == null ? null : (ims.framework.utils.beans.DateBean)vo.getDateNextChange().getBean();
		this.iscurrentlyactive = vo.getIsCurrentlyActive();
	}

	public ims.nursing.vo.BladderManagement buildVo()
	{
		return this.buildVo(new ims.vo.ValueObjectBeanMap());
	}

	public ims.nursing.vo.BladderManagement buildVo(ims.vo.ValueObjectBeanMap map)
	{
		ims.nursing.vo.BladderManagement vo = null;
		if(map != null)
			vo = (ims.nursing.vo.BladderManagement)map.getValueObject(this);
		if(vo == null)
		{
			vo = new ims.nursing.vo.BladderManagement();
			map.addValueObject(this, vo);
			vo.populate(map, this);
		}
		return vo;
	}

	public Integer getId()
	{
		return this.id;
	}
	public void setId(Integer value)
	{
		this.id = value;
	}
	public int getVersion()
	{
		return this.version;
	}
	public void setVersion(int value)
	{
		this.version = value;
	}
	public ims.vo.LookupInstanceBean getMethod()
	{
		return this.method;
	}
	public void setMethod(ims.vo.LookupInstanceBean value)
	{
		this.method = value;
	}
	public ims.vo.LookupInstanceBean getType()
	{
		return this.type;
	}
	public void setType(ims.vo.LookupInstanceBean value)
	{
		this.type = value;
	}
	public Integer getSize()
	{
		return this.size;
	}
	public void setSize(Integer value)
	{
		this.size = value;
	}
	public ims.framework.utils.beans.DateBean getDateInserted()
	{
		return this.dateinserted;
	}
	public void setDateInserted(ims.framework.utils.beans.DateBean value)
	{
		this.dateinserted = value;
	}
	public ims.framework.utils.beans.DateBean getDateNextChange()
	{
		return this.datenextchange;
	}
	public void setDateNextChange(ims.framework.utils.beans.DateBean value)
	{
		this.datenextchange = value;
	}
	public Boolean getIsCurrentlyActive()
	{
		return this.iscurrentlyactive;
	}
	public void setIsCurrentlyActive(Boolean value)
	{
		this.iscurrentlyactive = value;
	}

	private Integer id;
	private int version;
	private ims.vo.LookupInstanceBean method;
	private ims.vo.LookupInstanceBean type;
	private Integer size;
	private ims.framework.utils.beans.DateBean dateinserted;
	private ims.framework.utils.beans.DateBean datenextchange;
	private Boolean iscurrentlyactive;
}
