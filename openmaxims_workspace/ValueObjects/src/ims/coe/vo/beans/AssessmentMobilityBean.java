//#############################################################################
//#                                                                           #
//#  Copyright (C) <2014>  <IMS MAXIMS>                                       #
//#                                                                           #
//#  This program is free software: you can redistribute it and/or modify     #
//#  it under the terms of the GNU Affero General Public License as           #
//#  published by the Free Software Foundation, either version 3 of the       #
//#  License, or (at your option) any later version.                          # 
//#                                                                           #
//#  This program is distributed in the hope that it will be useful,          #
//#  but WITHOUT ANY WARRANTY; without even the implied warranty of           #
//#  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the            #
//#  GNU Affero General Public License for more details.                      #
//#                                                                           #
//#  You should have received a copy of the GNU Affero General Public License #
//#  along with this program.  If not, see <http://www.gnu.org/licenses/>.    #
//#                                                                           #
//#############################################################################
//#EOH
// This code was generated by Barbara Worwood using IMS Development Environment (version 1.80 build 5007.25751)
// Copyright (C) 1995-2014 IMS MAXIMS. All rights reserved.
// WARNING: DO NOT MODIFY the content of this file

package ims.coe.vo.beans;

public class AssessmentMobilityBean extends ims.vo.ValueObjectBean
{
	public AssessmentMobilityBean()
	{
	}
	public AssessmentMobilityBean(ims.coe.vo.AssessmentMobility vo)
	{
		this.id = vo.getBoId();
		this.version = vo.getBoVersion();
		this.iscomplete = vo.getIsComplete();
		this.assessmentinfo = vo.getAssessmentInfo() == null ? null : vo.getAssessmentInfo().getBeanCollection();
		this.copy = vo.getCopy();
		this.componenttype = vo.getComponentType() == null ? null : (ims.vo.LookupInstanceBean)vo.getComponentType().getBean();
		this.lastassessmentinfo = vo.getLastAssessmentInfo() == null ? null : (ims.nursing.vo.beans.AssessmentInfoBean)vo.getLastAssessmentInfo().getBean();
		this.careplantemplate = vo.getCarePlanTemplate() == null ? null : vo.getCarePlanTemplate().getBeanCollection();
		this.careplans = vo.getCarePlans() == null ? null : vo.getCarePlans().getBeanCollection();
		this.optionalfallassessment = vo.getOptionalFallAssessment() == null ? null : (ims.coe.vo.beans.FallAssessmentBean)vo.getOptionalFallAssessment().getBean();
		this.assistancewithmobility = vo.getAssistanceWithMobility() == null ? null : (ims.vo.LookupInstanceBean)vo.getAssistanceWithMobility().getBean();
		this.notes = vo.getNotes();
		this.bedtype = vo.getBedType() == null ? null : (ims.vo.LookupInstanceBean)vo.getBedType().getBean();
		this.chairtype = vo.getChairType() == null ? null : (ims.vo.LookupInstanceBean)vo.getChairType().getBean();
		this.patienthandling = vo.getPatientHandling() == null ? null : (ims.coe.vo.beans.PatientHandlingHeaderBean)vo.getPatientHandling().getBean();
		this.patientmovement = vo.getPatientMovement() == null ? null : (ims.coe.vo.beans.PatientMovementHeaderBean)vo.getPatientMovement().getBean();
		this.fallassessmentscore = vo.getFallAssessmentScore();
		this.environmentchecklist = vo.getEnvironmentChecklist() == null ? null : (ims.nursing.vo.beans.EnvironmentalChecklistVoBean)vo.getEnvironmentChecklist().getBean();
	}

	public void populate(ims.vo.ValueObjectBeanMap map, ims.coe.vo.AssessmentMobility vo)
	{
		this.id = vo.getBoId();
		this.version = vo.getBoVersion();
		this.iscomplete = vo.getIsComplete();
		this.assessmentinfo = vo.getAssessmentInfo() == null ? null : vo.getAssessmentInfo().getBeanCollection();
		this.copy = vo.getCopy();
		this.componenttype = vo.getComponentType() == null ? null : (ims.vo.LookupInstanceBean)vo.getComponentType().getBean();
		this.lastassessmentinfo = vo.getLastAssessmentInfo() == null ? null : (ims.nursing.vo.beans.AssessmentInfoBean)vo.getLastAssessmentInfo().getBean(map);
		this.careplantemplate = vo.getCarePlanTemplate() == null ? null : vo.getCarePlanTemplate().getBeanCollection();
		this.careplans = vo.getCarePlans() == null ? null : vo.getCarePlans().getBeanCollection();
		this.optionalfallassessment = vo.getOptionalFallAssessment() == null ? null : (ims.coe.vo.beans.FallAssessmentBean)vo.getOptionalFallAssessment().getBean(map);
		this.assistancewithmobility = vo.getAssistanceWithMobility() == null ? null : (ims.vo.LookupInstanceBean)vo.getAssistanceWithMobility().getBean();
		this.notes = vo.getNotes();
		this.bedtype = vo.getBedType() == null ? null : (ims.vo.LookupInstanceBean)vo.getBedType().getBean();
		this.chairtype = vo.getChairType() == null ? null : (ims.vo.LookupInstanceBean)vo.getChairType().getBean();
		this.patienthandling = vo.getPatientHandling() == null ? null : (ims.coe.vo.beans.PatientHandlingHeaderBean)vo.getPatientHandling().getBean(map);
		this.patientmovement = vo.getPatientMovement() == null ? null : (ims.coe.vo.beans.PatientMovementHeaderBean)vo.getPatientMovement().getBean(map);
		this.fallassessmentscore = vo.getFallAssessmentScore();
		this.environmentchecklist = vo.getEnvironmentChecklist() == null ? null : (ims.nursing.vo.beans.EnvironmentalChecklistVoBean)vo.getEnvironmentChecklist().getBean(map);
	}

	public ims.coe.vo.AssessmentMobility buildVo()
	{
		return this.buildVo(new ims.vo.ValueObjectBeanMap());
	}

	public ims.coe.vo.AssessmentMobility buildVo(ims.vo.ValueObjectBeanMap map)
	{
		ims.coe.vo.AssessmentMobility vo = null;
		if(map != null)
			vo = (ims.coe.vo.AssessmentMobility)map.getValueObject(this);
		if(vo == null)
		{
			vo = new ims.coe.vo.AssessmentMobility();
			map.addValueObject(this, vo);
			vo.populate(map, this);
		}
		return vo;
	}

	public Integer getId()
	{
		return this.id;
	}
	public void setId(Integer value)
	{
		this.id = value;
	}
	public int getVersion()
	{
		return this.version;
	}
	public void setVersion(int value)
	{
		this.version = value;
	}
	public Boolean getIsComplete()
	{
		return this.iscomplete;
	}
	public void setIsComplete(Boolean value)
	{
		this.iscomplete = value;
	}
	public ims.nursing.vo.beans.AssessmentInfoBean[] getAssessmentInfo()
	{
		return this.assessmentinfo;
	}
	public void setAssessmentInfo(ims.nursing.vo.beans.AssessmentInfoBean[] value)
	{
		this.assessmentinfo = value;
	}
	public Boolean getCopy()
	{
		return this.copy;
	}
	public void setCopy(Boolean value)
	{
		this.copy = value;
	}
	public ims.vo.LookupInstanceBean getComponentType()
	{
		return this.componenttype;
	}
	public void setComponentType(ims.vo.LookupInstanceBean value)
	{
		this.componenttype = value;
	}
	public ims.nursing.vo.beans.AssessmentInfoBean getLastAssessmentInfo()
	{
		return this.lastassessmentinfo;
	}
	public void setLastAssessmentInfo(ims.nursing.vo.beans.AssessmentInfoBean value)
	{
		this.lastassessmentinfo = value;
	}
	public ims.nursing.vo.beans.CarePlanTemplateBean[] getCarePlanTemplate()
	{
		return this.careplantemplate;
	}
	public void setCarePlanTemplate(ims.nursing.vo.beans.CarePlanTemplateBean[] value)
	{
		this.careplantemplate = value;
	}
	public ims.nursing.vo.beans.CarePlanBean[] getCarePlans()
	{
		return this.careplans;
	}
	public void setCarePlans(ims.nursing.vo.beans.CarePlanBean[] value)
	{
		this.careplans = value;
	}
	public ims.coe.vo.beans.FallAssessmentBean getOptionalFallAssessment()
	{
		return this.optionalfallassessment;
	}
	public void setOptionalFallAssessment(ims.coe.vo.beans.FallAssessmentBean value)
	{
		this.optionalfallassessment = value;
	}
	public ims.vo.LookupInstanceBean getAssistanceWithMobility()
	{
		return this.assistancewithmobility;
	}
	public void setAssistanceWithMobility(ims.vo.LookupInstanceBean value)
	{
		this.assistancewithmobility = value;
	}
	public String getNotes()
	{
		return this.notes;
	}
	public void setNotes(String value)
	{
		this.notes = value;
	}
	public ims.vo.LookupInstanceBean getBedType()
	{
		return this.bedtype;
	}
	public void setBedType(ims.vo.LookupInstanceBean value)
	{
		this.bedtype = value;
	}
	public ims.vo.LookupInstanceBean getChairType()
	{
		return this.chairtype;
	}
	public void setChairType(ims.vo.LookupInstanceBean value)
	{
		this.chairtype = value;
	}
	public ims.coe.vo.beans.PatientHandlingHeaderBean getPatientHandling()
	{
		return this.patienthandling;
	}
	public void setPatientHandling(ims.coe.vo.beans.PatientHandlingHeaderBean value)
	{
		this.patienthandling = value;
	}
	public ims.coe.vo.beans.PatientMovementHeaderBean getPatientMovement()
	{
		return this.patientmovement;
	}
	public void setPatientMovement(ims.coe.vo.beans.PatientMovementHeaderBean value)
	{
		this.patientmovement = value;
	}
	public Integer getFallAssessmentScore()
	{
		return this.fallassessmentscore;
	}
	public void setFallAssessmentScore(Integer value)
	{
		this.fallassessmentscore = value;
	}
	public ims.nursing.vo.beans.EnvironmentalChecklistVoBean getEnvironmentChecklist()
	{
		return this.environmentchecklist;
	}
	public void setEnvironmentChecklist(ims.nursing.vo.beans.EnvironmentalChecklistVoBean value)
	{
		this.environmentchecklist = value;
	}

	private Integer id;
	private int version;
	private Boolean iscomplete;
	private ims.nursing.vo.beans.AssessmentInfoBean[] assessmentinfo;
	private Boolean copy;
	private ims.vo.LookupInstanceBean componenttype;
	private ims.nursing.vo.beans.AssessmentInfoBean lastassessmentinfo;
	private ims.nursing.vo.beans.CarePlanTemplateBean[] careplantemplate;
	private ims.nursing.vo.beans.CarePlanBean[] careplans;
	private ims.coe.vo.beans.FallAssessmentBean optionalfallassessment;
	private ims.vo.LookupInstanceBean assistancewithmobility;
	private String notes;
	private ims.vo.LookupInstanceBean bedtype;
	private ims.vo.LookupInstanceBean chairtype;
	private ims.coe.vo.beans.PatientHandlingHeaderBean patienthandling;
	private ims.coe.vo.beans.PatientMovementHeaderBean patientmovement;
	private Integer fallassessmentscore;
	private ims.nursing.vo.beans.EnvironmentalChecklistVoBean environmentchecklist;
}
