//#############################################################################
//#                                                                           #
//#  Copyright (C) <2014>  <IMS MAXIMS>                                       #
//#                                                                           #
//#  This program is free software: you can redistribute it and/or modify     #
//#  it under the terms of the GNU Affero General Public License as           #
//#  published by the Free Software Foundation, either version 3 of the       #
//#  License, or (at your option) any later version.                          # 
//#                                                                           #
//#  This program is distributed in the hope that it will be useful,          #
//#  but WITHOUT ANY WARRANTY; without even the implied warranty of           #
//#  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the            #
//#  GNU Affero General Public License for more details.                      #
//#                                                                           #
//#  You should have received a copy of the GNU Affero General Public License #
//#  along with this program.  If not, see <http://www.gnu.org/licenses/>.    #
//#                                                                           #
//#############################################################################
//#EOH
// This code was generated by Barbara Worwood using IMS Development Environment (version 1.80 build 5007.25751)
// Copyright (C) 1995-2014 IMS MAXIMS. All rights reserved.
// WARNING: DO NOT MODIFY the content of this file

package ims.clinical.vo.beans;

public class PatientGoalTargetVoBean extends ims.vo.ValueObjectBean
{
	public PatientGoalTargetVoBean()
	{
	}
	public PatientGoalTargetVoBean(ims.clinical.vo.PatientGoalTargetVo vo)
	{
		this.id = vo.getBoId();
		this.version = vo.getBoVersion();
		this.target = vo.getTarget();
		this.targetdate = vo.getTargetDate() == null ? null : (ims.framework.utils.beans.DateBean)vo.getTargetDate().getBean();
		this.dateachieved = vo.getDateAchieved() == null ? null : (ims.framework.utils.beans.DateBean)vo.getDateAchieved().getBean();
		this.loa = vo.getLOA() == null ? null : (ims.vo.LookupInstanceBean)vo.getLOA().getBean();
		this.reasonfornona = vo.getReasonForNonA();
	}

	public void populate(ims.vo.ValueObjectBeanMap map, ims.clinical.vo.PatientGoalTargetVo vo)
	{
		this.id = vo.getBoId();
		this.version = vo.getBoVersion();
		this.target = vo.getTarget();
		this.targetdate = vo.getTargetDate() == null ? null : (ims.framework.utils.beans.DateBean)vo.getTargetDate().getBean();
		this.dateachieved = vo.getDateAchieved() == null ? null : (ims.framework.utils.beans.DateBean)vo.getDateAchieved().getBean();
		this.loa = vo.getLOA() == null ? null : (ims.vo.LookupInstanceBean)vo.getLOA().getBean();
		this.reasonfornona = vo.getReasonForNonA();
	}

	public ims.clinical.vo.PatientGoalTargetVo buildVo()
	{
		return this.buildVo(new ims.vo.ValueObjectBeanMap());
	}

	public ims.clinical.vo.PatientGoalTargetVo buildVo(ims.vo.ValueObjectBeanMap map)
	{
		ims.clinical.vo.PatientGoalTargetVo vo = null;
		if(map != null)
			vo = (ims.clinical.vo.PatientGoalTargetVo)map.getValueObject(this);
		if(vo == null)
		{
			vo = new ims.clinical.vo.PatientGoalTargetVo();
			map.addValueObject(this, vo);
			vo.populate(map, this);
		}
		return vo;
	}

	public Integer getId()
	{
		return this.id;
	}
	public void setId(Integer value)
	{
		this.id = value;
	}
	public int getVersion()
	{
		return this.version;
	}
	public void setVersion(int value)
	{
		this.version = value;
	}
	public String getTarget()
	{
		return this.target;
	}
	public void setTarget(String value)
	{
		this.target = value;
	}
	public ims.framework.utils.beans.DateBean getTargetDate()
	{
		return this.targetdate;
	}
	public void setTargetDate(ims.framework.utils.beans.DateBean value)
	{
		this.targetdate = value;
	}
	public ims.framework.utils.beans.DateBean getDateAchieved()
	{
		return this.dateachieved;
	}
	public void setDateAchieved(ims.framework.utils.beans.DateBean value)
	{
		this.dateachieved = value;
	}
	public ims.vo.LookupInstanceBean getLOA()
	{
		return this.loa;
	}
	public void setLOA(ims.vo.LookupInstanceBean value)
	{
		this.loa = value;
	}
	public String getReasonForNonA()
	{
		return this.reasonfornona;
	}
	public void setReasonForNonA(String value)
	{
		this.reasonfornona = value;
	}

	private Integer id;
	private int version;
	private String target;
	private ims.framework.utils.beans.DateBean targetdate;
	private ims.framework.utils.beans.DateBean dateachieved;
	private ims.vo.LookupInstanceBean loa;
	private String reasonfornona;
}
