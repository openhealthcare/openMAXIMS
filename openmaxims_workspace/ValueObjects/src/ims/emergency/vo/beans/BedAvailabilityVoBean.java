//#############################################################################
//#                                                                           #
//#  Copyright (C) <2014>  <IMS MAXIMS>                                       #
//#                                                                           #
//#  This program is free software: you can redistribute it and/or modify     #
//#  it under the terms of the GNU Affero General Public License as           #
//#  published by the Free Software Foundation, either version 3 of the       #
//#  License, or (at your option) any later version.                          # 
//#                                                                           #
//#  This program is distributed in the hope that it will be useful,          #
//#  but WITHOUT ANY WARRANTY; without even the implied warranty of           #
//#  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the            #
//#  GNU Affero General Public License for more details.                      #
//#                                                                           #
//#  You should have received a copy of the GNU Affero General Public License #
//#  along with this program.  If not, see <http://www.gnu.org/licenses/>.    #
//#                                                                           #
//#############################################################################
//#EOH
// This code was generated by Barbara Worwood using IMS Development Environment (version 1.80 build 5007.25751)
// Copyright (C) 1995-2014 IMS MAXIMS. All rights reserved.
// WARNING: DO NOT MODIFY the content of this file

package ims.emergency.vo.beans;

public class BedAvailabilityVoBean extends ims.vo.ValueObjectBean
{
	public BedAvailabilityVoBean()
	{
	}
	public BedAvailabilityVoBean(ims.emergency.vo.BedAvailabilityVo vo)
	{
		this.id = vo.getBoId();
		this.version = vo.getBoVersion();
		this.patient = vo.getPatient() == null ? null : new ims.vo.RefVoBean(vo.getPatient().getBoId(), vo.getPatient().getBoVersion());
		this.episode = vo.getEpisode() == null ? null : new ims.vo.RefVoBean(vo.getEpisode().getBoId(), vo.getEpisode().getBoVersion());
		this.attendance = vo.getAttendance() == null ? null : new ims.vo.RefVoBean(vo.getAttendance().getBoId(), vo.getAttendance().getBoVersion());
		this.trackingarea = vo.getTrackingArea() == null ? null : new ims.vo.RefVoBean(vo.getTrackingArea().getBoId(), vo.getTrackingArea().getBoVersion());
		this.awaitingdatetime = vo.getAwaitingDateTime() == null ? null : (ims.framework.utils.beans.DateTimeBean)vo.getAwaitingDateTime().getBean();
		this.availabledatetime = vo.getAvailableDateTime() == null ? null : (ims.framework.utils.beans.DateTimeBean)vo.getAvailableDateTime().getBean();
		this.ward = vo.getWard() == null ? null : (ims.core.vo.beans.LocationLiteVoBean)vo.getWard().getBean();
		this.requestedby = vo.getRequestedBy() == null ? null : (ims.core.vo.beans.MemberOfStaffLiteVoBean)vo.getRequestedBy().getBean();
	}

	public void populate(ims.vo.ValueObjectBeanMap map, ims.emergency.vo.BedAvailabilityVo vo)
	{
		this.id = vo.getBoId();
		this.version = vo.getBoVersion();
		this.patient = vo.getPatient() == null ? null : new ims.vo.RefVoBean(vo.getPatient().getBoId(), vo.getPatient().getBoVersion());
		this.episode = vo.getEpisode() == null ? null : new ims.vo.RefVoBean(vo.getEpisode().getBoId(), vo.getEpisode().getBoVersion());
		this.attendance = vo.getAttendance() == null ? null : new ims.vo.RefVoBean(vo.getAttendance().getBoId(), vo.getAttendance().getBoVersion());
		this.trackingarea = vo.getTrackingArea() == null ? null : new ims.vo.RefVoBean(vo.getTrackingArea().getBoId(), vo.getTrackingArea().getBoVersion());
		this.awaitingdatetime = vo.getAwaitingDateTime() == null ? null : (ims.framework.utils.beans.DateTimeBean)vo.getAwaitingDateTime().getBean();
		this.availabledatetime = vo.getAvailableDateTime() == null ? null : (ims.framework.utils.beans.DateTimeBean)vo.getAvailableDateTime().getBean();
		this.ward = vo.getWard() == null ? null : (ims.core.vo.beans.LocationLiteVoBean)vo.getWard().getBean(map);
		this.requestedby = vo.getRequestedBy() == null ? null : (ims.core.vo.beans.MemberOfStaffLiteVoBean)vo.getRequestedBy().getBean(map);
	}

	public ims.emergency.vo.BedAvailabilityVo buildVo()
	{
		return this.buildVo(new ims.vo.ValueObjectBeanMap());
	}

	public ims.emergency.vo.BedAvailabilityVo buildVo(ims.vo.ValueObjectBeanMap map)
	{
		ims.emergency.vo.BedAvailabilityVo vo = null;
		if(map != null)
			vo = (ims.emergency.vo.BedAvailabilityVo)map.getValueObject(this);
		if(vo == null)
		{
			vo = new ims.emergency.vo.BedAvailabilityVo();
			map.addValueObject(this, vo);
			vo.populate(map, this);
		}
		return vo;
	}

	public Integer getId()
	{
		return this.id;
	}
	public void setId(Integer value)
	{
		this.id = value;
	}
	public int getVersion()
	{
		return this.version;
	}
	public void setVersion(int value)
	{
		this.version = value;
	}
	public ims.vo.RefVoBean getPatient()
	{
		return this.patient;
	}
	public void setPatient(ims.vo.RefVoBean value)
	{
		this.patient = value;
	}
	public ims.vo.RefVoBean getEpisode()
	{
		return this.episode;
	}
	public void setEpisode(ims.vo.RefVoBean value)
	{
		this.episode = value;
	}
	public ims.vo.RefVoBean getAttendance()
	{
		return this.attendance;
	}
	public void setAttendance(ims.vo.RefVoBean value)
	{
		this.attendance = value;
	}
	public ims.vo.RefVoBean getTrackingArea()
	{
		return this.trackingarea;
	}
	public void setTrackingArea(ims.vo.RefVoBean value)
	{
		this.trackingarea = value;
	}
	public ims.framework.utils.beans.DateTimeBean getAwaitingDateTime()
	{
		return this.awaitingdatetime;
	}
	public void setAwaitingDateTime(ims.framework.utils.beans.DateTimeBean value)
	{
		this.awaitingdatetime = value;
	}
	public ims.framework.utils.beans.DateTimeBean getAvailableDateTime()
	{
		return this.availabledatetime;
	}
	public void setAvailableDateTime(ims.framework.utils.beans.DateTimeBean value)
	{
		this.availabledatetime = value;
	}
	public ims.core.vo.beans.LocationLiteVoBean getWard()
	{
		return this.ward;
	}
	public void setWard(ims.core.vo.beans.LocationLiteVoBean value)
	{
		this.ward = value;
	}
	public ims.core.vo.beans.MemberOfStaffLiteVoBean getRequestedBy()
	{
		return this.requestedby;
	}
	public void setRequestedBy(ims.core.vo.beans.MemberOfStaffLiteVoBean value)
	{
		this.requestedby = value;
	}

	private Integer id;
	private int version;
	private ims.vo.RefVoBean patient;
	private ims.vo.RefVoBean episode;
	private ims.vo.RefVoBean attendance;
	private ims.vo.RefVoBean trackingarea;
	private ims.framework.utils.beans.DateTimeBean awaitingdatetime;
	private ims.framework.utils.beans.DateTimeBean availabledatetime;
	private ims.core.vo.beans.LocationLiteVoBean ward;
	private ims.core.vo.beans.MemberOfStaffLiteVoBean requestedby;
}
