 ...
 */
public class ICPActionCloneVo extends ims.icp.vo.ICPActionLiteVo implements ims.vo.ImsCloneable, Comparable
{
	private static final long serialVersionUID = 1L;

	public ICPActionCloneVo()
	{
	}
	public ICPActionCloneVo(Integer id, int version)
	{
		super(id, version);
	}
	public ICPActionCloneVo(ims.icp.vo.beans.ICPActionCloneVoBean bean)
	{
		this.id = bean.getId();
	...
	public void populate(ims.vo.ValueObjectBeanMap map, ims.icp.vo.beans.ICPActionCloneVoBean bean)
	{
		this.id = bean.getId();
		this.version = bean.getVersion();
		this.name = bean.getName();
	...
	{
		ims.icp.vo.beans.ICPActionCloneVoBean bean = null;
		if(map != null)
			bean = (ims.icp.vo.beans.ICPActionCloneVoBean)map.getValueObjectBean(this);
		if (bean == null)
		{
			bean = new ims.icp.vo.beans.ICPActionCloneVoBean();
	...
	/**
	* public ICPActionShortVo getCloneActionShortVo()
	*/
	/**
	 * WDEV-13773
	 * <br>Function used to create an ICP Phase Full clone from current VO
	 * <br>Will not contain the clinical details linked actions
	 */
	public ICPActionShortVo getCloneActionShortVo()
	{
		// Create ICP Action Short
		ICPActionShortVo shortAction = new ICPActionShortVo(this.id, this.version);
		
		// Copy simple fields
		shortAction.setName(this.name);
		shortAction.setDescription(this.description);
		shortAction.setSequence(this.sequence);
		shortAction.setDiscipline(this.discipline);
		shortAction.setStatus(this.status);
		shortAction.setIsMandatory(this.ismandatory);
		shortAction.setIsActivatedOnInitiation(this.isactivatedoninitiation);
		shortAction.setHelpURL(this.helpurl);
		shortAction.setLinkedType(this.linkedtype);
		shortAction.setRequiresCounterSignature(this.requirescountersignature);
		
		shortAction.setLinkedForm(this.linkedform);
		shortAction.setLinkedAssessment(this.linkedassessment);
		shortAction.setLinkedGraphicAssessment(this.linkedgraphicassessment);
		shortAction.setLinkedOrderEntryTemplate(this.linkedorderentrytemplate);
		
		// Clinical details linked actions collection will be empty
		shortAction.setClinicalDetailActions(new ims.icps.configuration.vo.ICPActionRefVoCollection());
	...
	public String[] validate(String[] existingErrors)
	{
		...
		if(this.name == null || this.name.length() == 0)
			...
			listOfErrors.add("The length of the field [name] in the value object [ims.icp.vo.ICPActionCloneVo] is too big. It should be less or equal to 255");
		if(this.description != null)
			if(this.description.length() > 255)
				listOfErrors.add("The length of the field [description] in the value object [ims.icp.vo.ICPActionCloneVo] is too big. It should be less or equal to 255");
		...
		if(this.helpurl != null)
			if(this.helpurl.length() > 255)
				listOfErrors.add("The length of the field [helpurl] in the value object [ims.icp.vo.ICPActionCloneVo] is too big. It should be less or equal to 255");
	...
	public Object clone()
	{
		...
		ICPActionCloneVo clone = new ICPActionCloneVo(this.id, this.version);
	...
	public int compareTo(Object obj, boolean caseInsensitive)
	{
		...
		if (!(ICPActionCloneVo.class.isAssignableFrom(obj.getClass())))
		{
			throw new ClassCastException("A ICPActionCloneVo object cannot be compared an Object of type " + obj.getClass().getName());
		}
		if (this.id == null)
			return 1;
		if (((ICPActionCloneVo)obj).getBoId() == null)
			return -1;
		return this.id.compareTo(((ICPActionCloneVo)obj).getBoId());
	}
