...
public final class Care_spell
{
	private Care_spellFilter lastGetFilter = null;
	private final String serviceName = "CARE_SPELL";
	 ...
	 */ 
	public Care_spellFilter Filter = new Care_spellFilter();			
	/**
	 * Represents the filter used by the current Data Transfer Object to Insert/Update data.
	 */ 
	public Care_spellEditFilter EditFilter = new Care_spellEditFilter();
	/**
	 * Contains the data records for the current Data Transfer Object
	 */ 
	public Care_spellCollection DataCollection = new Care_spellCollection();

	/**
	 * Creates a new Care_spell Data Transfer Object.
	 */ 
	public Care_spell(ims.dto.Connection connection) throws ims.dto.Exception
	{	
		if(connection == null)
	 ...
	 */
	public Care_spell cloneObject() throws ims.dto.Exception
	{
		Care_spell cloneObject = new Care_spell(Connection);
		...
		for(int x = 0; x < DataCollection.count(); x++)
		{
			...
			cloneObject.DataCollection.get(index).Root_epid = DataCollection.get(x).Root_epid;
			cloneObject.DataCollection.get(index).Start_date = DataCollection.get(x).Start_date;
	...
	{
		return "Care_spell.imx".toLowerCase();
	}
	/**
	...
	public ims.dto.Result count()
	{
		...
		if(reLoginResult != null)
			return new ims.dto.Result(reLoginResult.getMessage(), "DTO.Client.Care_spell.Count");
			
		int result = Connection.count(serviceName, encodeNASFilter());
		if(result >= 0)
			return new ims.dto.Result(result, "No error detected. The count result is held in the ID field", "DTO.Client.Care_spell.Count");
	...
	*/
	...
	{	
		...
		ims.dto.Result reLoginResult = Connection.reLogin();
		if(reLoginResult != null)
			return new ims.dto.Result(reLoginResult.getMessage(), "DTO.Client.Care_spell.Get");
	...
	*/
	...
	{						
		ims.dto.Result reLoginResult = Connection.reLogin();
		if(reLoginResult != null)
			return new ims.dto.Result(reLoginResult.getMessage(), "DTO.Client.Care_spell.GetForUpdate");
							
		if(lastGetFilter == null)	
			return new ims.dto.Result("Last get method failed or not called", "DTO.Client.Care_spell.GetForUpdate");
		...
		return new ims.dto.Result("The data was changed by another user", "DTO.Client.Care_spell.GetForUpdate");
	}		
	/**
	...
	*/
	public ims.dto.Result insert()
	{
		ims.dto.Result reLoginResult = Connection.reLogin();
		if(reLoginResult != null)
			return new ims.dto.Result(reLoginResult.getMessage(), "DTO.Client.Care_spell.Insert");
					
		if(DataCollection.count() == 0)
			return new ims.dto.Result("No data to insert", "DTO.Client.Care_spell.Insert");
			
		if(DataCollection.count() > 1)
			return new ims.dto.Result("Multiple object insert not allowed", "DTO.Client.Care_spell.Insert");
		...
		{
			return new ims.dto.Result("Invalid record ID returned", "DTO.Client.Care_spell.Insert");
		}
		
		return new ims.dto.Result(recordID, "No error. The ID of the new record is in the ID field", "DTO.Client.Care_spell.Insert");
	}
	/**
	...
	{
		if(action.length() == 0)
			return new ims.dto.Result("Invalid action name", "DTO.Client.Care_spell.ExecuteAction");
			
		ims.dto.Result reLoginResult = Connection.reLogin();
		if(reLoginResult != null)
			return new ims.dto.Result(reLoginResult.getMessage(), "DTO.Client.Care_spell.ExecuteAction");
		
		if(DataCollection.count() == 0)
			return new ims.dto.Result("Data container is empty", "DTO.Client.Care_spell.ExecuteAction");
			
		if(DataCollection.count() > 1)
			return new ims.dto.Result("Multiple objects are not allowed", "DTO.Client.Care_spell.ExecuteAction");
		...
		try
		{
			return new ims.dto.Result(new Integer(Connection.getValueAt(2)).intValue(), "No error", "DTO.Client.Care_spell.ExecuteAction");	
		}
		catch(NumberFormatException ex)
		{
			return new ims.dto.Result("Invalid server response", "DTO.Client.Care_spell.ExecuteAction");
		}
	}
	/**
	...
	{
		if(action.length() == 0)
			return new ims.dto.Result("Invalid action name", "DTO.Client.Care_spell.TransferData");
			
		ims.dto.Result reLoginResult = Connection.reLogin();
		if(reLoginResult != null)
			return new ims.dto.Result(reLoginResult.getMessage(), "DTO.Client.Care_spell.TransferData");
		
		if(DataCollection.count() == 0)
			return new ims.dto.Result("No data to transfer", "DTO.Client.Care_spell.TransferData");
			
		if(DataCollection.count() > 1)
			return new ims.dto.Result("Multiple objects not allowed", "DTO.Client.Care_spell.TransferData");
	...
	{			
		ims.dto.Result reLoginResult = Connection.reLogin();
		if(reLoginResult != null)
			return new ims.dto.Result(reLoginResult.getMessage(), "DTO.Client.Care_spell.Update");
				
		if(DataCollection.count() == 0)
			return new ims.dto.Result("No data to update", "DTO.Client.Care_spell.Update");	
			
		if(DataCollection.count() > 1)
			return new ims.dto.Result("Multiple object update not allowed", "DTO.Client.Care_spell.Update");
	...
	{
		if(!listInProgress)
			return new ims.dto.Result("No active list running", "DTO.Client.Care_spell.StopList");
	...
	{	
		...
		ims.dto.Result reLoginResult = Connection.reLogin();
		if(reLoginResult != null)
			return new ims.dto.Result(reLoginResult.getMessage(), "DTO.Client.Care_spell.List");
	...
	private String encodeNASFilter(Care_spellFilter filter)
	{
		if(filter == null)
			return "";
		...
		if(Filter.Care_spell_id != null && filter.Care_spell_id.length()> 0)
		{
			...
			filterString += "CARE_SPELL_ID" + ims.dto.NASMessageCodes.ATTRIBUTEVALUESEPARATOR + filter.Care_spell_id;
		}
		...
		if(Filter.Root_epid != null && filter.Root_epid.length()> 0)
		{
			if(filterString.length() > 0)
				filterString += ims.dto.NASMessageCodes.PAIRSEPARATOR;
			filterString += "ROOT_EPID" + ims.dto.NASMessageCodes.ATTRIBUTEVALUESEPARATOR + filter.Root_epid;
		}
		
		if(Filter.Start_date != null && filter.Start_date.length()> 0)
		{
			if(filterString.length() > 0)
				filterString += ims.dto.NASMessageCodes.PAIRSEPARATOR;
			filterString += "START_DATE" + ims.dto.NASMessageCodes.ATTRIBUTEVALUESEPARATOR + filter.Start_date;
		}
		...
		if(Filter.End_date != null && filter.End_date.length()> 0)
		{
			if(filterString.length() > 0)
				filterString += ims.dto.NASMessageCodes.PAIRSEPARATOR;
			filterString += "END_DATE" + ims.dto.NASMessageCodes.ATTRIBUTEVALUESEPARATOR + filter.End_date;
		}
	...
	private String encodeNASMessage()
	{
		...
		Care_spellRecord data = (Care_spellRecord)DataCollection.get(0);
		
		if(EditFilter.IncludeCare_spell_id)
		{
			if(dataString.length() > 0)
				dataString += ims.dto.NASMessageCodes.PAIRSEPARATOR;
			dataString += "CARE_SPELL_ID" + ims.dto.NASMessageCodes.ATTRIBUTEVALUESEPARATOR + Connection.encodeFieldValue(data.Care_spell_id);
		}
		...
		if(EditFilter.IncludeRoot_epid)
		{
			if(dataString.length() > 0)
				dataString += ims.dto.NASMessageCodes.PAIRSEPARATOR;
			dataString += "ROOT_EPID" + ims.dto.NASMessageCodes.ATTRIBUTEVALUESEPARATOR + Connection.encodeFieldValue(data.Root_epid);
		}
		
		if(EditFilter.IncludeStart_date)
		{
			if(dataString.length() > 0)
				dataString += ims.dto.NASMessageCodes.PAIRSEPARATOR;
			dataString += "START_DATE" + ims.dto.NASMessageCodes.ATTRIBUTEVALUESEPARATOR + Connection.encodeFieldValue(data.Start_date);
		}
		...
		if(EditFilter.IncludeEnd_date)
		{
			if(dataString.length() > 0)
				dataString += ims.dto.NASMessageCodes.PAIRSEPARATOR;
			dataString += "END_DATE" + ims.dto.NASMessageCodes.ATTRIBUTEVALUESEPARATOR + Connection.encodeFieldValue(data.End_date);
		}
	...
	private int decodeNASMessage()
	{
		...
		{
			Care_spellRecord record = new Care_spellRecord();
			...
			record.Root_epid = Connection.decodeFieldValue(Connection.getValueFor(valueItems, "ROOT_EPID"));
			record.Start_date = Connection.decodeFieldValue(Connection.getValueFor(valueItems, "START_DATE"));
	...
	public final class Care_spellCollection
	{
		private java.util.ArrayList recordCollection = new java.util.ArrayList();
		
		public int add()
		{
			Care_spellRecord newRecord = new Care_spellRecord();
			recordCollection.add(newRecord);
			return count() - 1;
		}
		public int add(Care_spellRecord record)
		{
		...
		public Care_spellRecord get(int index)
		{
			return (Care_spellRecord)recordCollection.get(index);
		}
	...
	public final class Care_spellRecord
	{
	...
	public final class Care_spellFilter
	{			
		public String Care_spell_id = "";
		public String Pkey = "";
		public String Rdat = "";
		...
		public Care_spellFilter cloneObject()
		{
			Care_spellFilter newFilter = new Care_spellFilter();
			...
			newFilter.Root_epid = this.Root_epid;
			newFilter.Start_date = this.Start_date;
	...
	public final class Care_spellEditFilter
	{			
		...
		public boolean IncludeRoot_epid = true;
		public boolean IncludeStart_date = true;
		...
		{				
			...
			IncludeCtim = true;
			IncludeModu = true;
			IncludeRoot_epid = true;
			IncludeStart_date = true;
		...
		{				
			...
			IncludeCtim = false;
			IncludeModu = false;
			IncludeRoot_epid = false;
			IncludeStart_date = false;
			IncludeEnd_date = false;
			IncludeActive = false;
			IncludeTstp = false;
		}
		public Care_spellEditFilter cloneObject()
		{
			Care_spellEditFilter newEditFilter = new Care_spellEditFilter();
			...
			newEditFilter.IncludeRoot_epid = this.IncludeRoot_epid;
			newEditFilter.IncludeStart_date = this.IncludeStart_date;
