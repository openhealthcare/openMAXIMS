...
public abstract class BaseLogic extends Handlers
{
	...
	{
		return ims.ocrr.domain.OrderSetEdit.class;
	}
	public final void setContext(ims.framework.UIEngine engine, GenForm form, ims.ocrr.domain.OrderSetEdit domain)
	{
	...
	protected final void bindgrdHelpTextcolTitleLookup()
	{
		this.form.grdHelpText().colTitleComboBox().clear();
		ims.ocrr.vo.lookups.HelpTextCategoryCollection lookupCollection = ims.ocrr.vo.lookups.LookupHelper.getHelpTextCategory(this.domain.getLookupService());
		for(int x = 0; x < lookupCollection.size(); x++)
		{
			this.form.grdHelpText().colTitleComboBox().newRow(lookupCollection.get(x), lookupCollection.get(x).getText(), lookupCollection.get(x).getImage(), lookupCollection.get(x).getTextColor());
		}
	}
	...
	{
		this.form.grdComponents().colStatusComboBox().clear();
		ims.core.vo.lookups.PreActiveActiveInactiveStatusCollection lookupCollection = ims.core.vo.lookups.LookupHelper.getPreActiveActiveInactiveStatus(this.domain.getLookupService());
		for(int x = 0; x < lookupCollection.size(); x++)
		{
			this.form.grdComponents().colStatusComboBox().newRow(lookupCollection.get(x), lookupCollection.get(x).getText(), lookupCollection.get(x).getImage(), lookupCollection.get(x).getTextColor());
		}
	}
	...
	protected ims.ocrr.domain.OrderSetEdit domain;
}
