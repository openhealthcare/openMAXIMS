...
public abstract class BaseLogic extends Handlers
{
	...
	{
		return ims.ocrr.domain.OrderEntryTemplateEdit.class;
	}
	public final void setContext(ims.framework.UIEngine engine, GenForm form, ims.ocrr.domain.OrderEntryTemplateEdit domain)
	{
	...
	protected final void oncmbCategoryValueSet(Object value)
	{
		...
		if(value == null)
		{
			if(listOfValues != null && listOfValues.size() > 0)
			{
				for(int x = 0; x < listOfValues.size(); x++)
				{
					ims.ocrr.vo.lookups.TemplateCategories existingInstance = (ims.ocrr.vo.lookups.TemplateCategories)listOfValues.get(x);
					if(!existingInstance.isActive())
					{
		...
		else if(value instanceof ims.ocrr.vo.lookups.TemplateCategories)
		{
			ims.ocrr.vo.lookups.TemplateCategories instance = (ims.ocrr.vo.lookups.TemplateCategories)value;
	...
	protected final void bindcmbCategoryLookup()
	{
		...
		ims.ocrr.vo.lookups.TemplateCategoriesCollection lookupCollection = ims.ocrr.vo.lookups.LookupHelper.getTemplateCategories(this.domain.getLookupService());
		for(int x = 0; x < lookupCollection.size(); x++)
		{
	...
	{
		ims.ocrr.vo.lookups.TemplateCategories instance = ims.ocrr.vo.lookups.LookupHelper.getTemplateCategoriesInstance(this.domain.getLookupService(), id);
		if(instance != null)
			this.form.cmbCategory().setValue(instance);
	}
	protected final void defaultcmbCategoryLookupValue()
	{
		this.form.cmbCategory().setValue((ims.ocrr.vo.lookups.TemplateCategories)domain.getLookupService().getDefaultInstance(ims.ocrr.vo.lookups.TemplateCategories.class, engine.getFormName().getID(), ims.ocrr.vo.lookups.TemplateCategories.TYPE_ID));
	}
	protected final void bindgrdComponentscolStatusLookup()
	{
		this.form.lyrDetails().tabComponents().grdComponents().colStatusComboBox().clear();
		ims.core.vo.lookups.PreActiveActiveInactiveStatusCollection lookupCollection = ims.core.vo.lookups.LookupHelper.getPreActiveActiveInactiveStatus(this.domain.getLookupService());
		for(int x = 0; x < lookupCollection.size(); x++)
		{
			this.form.lyrDetails().tabComponents().grdComponents().colStatusComboBox().newRow(lookupCollection.get(x), lookupCollection.get(x).getText(), lookupCollection.get(x).getImage(), lookupCollection.get(x).getTextColor());
		}
	}
	...
	protected ims.ocrr.domain.OrderEntryTemplateEdit domain;
}
