...
public class DED extends ims.domain.DomainObject implements ims.domain.SystemInformationRetainer, java.io.Serializable {
	public static final int CLASSID = 1091100005;
	private static final long serialVersionUID = 1091100005L;
    ...
    public DED (Integer id, int ver, Boolean includeRecord)
    {
	...
	public String toAuditString()
	{
		...
		{
		int i3=0;
		for (i3=0; i3<codeMappings.size(); i3++)
		{
			if (i3 > 0)
				auditStr.append(",");
			ims.core.clinical.domain.objects.TaxonomyMap obj = (ims.core.clinical.domain.objects.TaxonomyMap)codeMappings.get(i3);
		    if (obj != null)
			{
				if (i3 == 0)
				{
	...
	private static void fillSetFromXMLString(java.util.Set set, org.dom4j.Element el, ims.domain.DomainFactory factory, java.util.HashMap domMap) throws Exception
	{
		...
		for(int i=0; i<size; i++) 
		{
			...
			DED domainObject = getDEDfromXML(itemEl, factory, domMap);
	...
	public static DED getDEDfromXML(String xml, ims.domain.DomainFactory factory, java.util.HashMap domMap) throws Exception
	{
		org.dom4j.Document doc = new org.dom4j.io.SAXReader().read(new org.xml.sax.InputSource(xml));
		return getDEDfromXML(doc.getRootElement(), factory, domMap);
	}
	
	public static DED getDEDfromXML(org.dom4j.Element el, ims.domain.DomainFactory factory, java.util.HashMap domMap) throws Exception
	{
		if (el == null)
			return null;
		
		String className = el.attributeValue("type");
		if (!DED.class.getName().equals(className))
		{
			Class clz = Class.forName(className);
			if (!DED.class.isAssignableFrom(clz))
				throw new Exception("Element of type = " + className + " cannot be imported using the DED class");
			String shortClassName = className.substring(className.lastIndexOf(".")+1);
			String methodName = "get" + shortClassName + "fromXML";
			java.lang.reflect.Method m = clz.getMethod(methodName, new Class[]{org.dom4j.Element.class, ims.domain.DomainFactory.class, java.util.HashMap.class});
			return (DED)m.invoke(null, new Object[]{el, factory, domMap});
		}

		String impVersion = el.attributeValue("classVersion");
		if(!impVersion.equals(DED.CLASSVERSION))
		{
		...
		ret = (DED)factory.getImportedDomainObject(DED.class, externalSource, extId);	
		if (ret == null)
		{
			ret = new DED();
		}
		...
		if (impObj != null)
		{
			return (DED)impObj.getDomainObject();
		}
		else
		{
    		impObj = new ims.configuration.ImportedObject();
	...
	public static void fillFieldsfromXML(org.dom4j.Element el, ims.domain.DomainFactory factory, DED obj, java.util.HashMap domMap) throws Exception
	{
