...
public class EDDischargeSummarySchedule extends ims.domain.DomainObject implements ims.domain.SystemInformationRetainer, java.io.Serializable {
	public static final int CLASSID = 1028100069;
	private static final long serialVersionUID = 1028100069L;
    ...
    public EDDischargeSummarySchedule ()
    {
    	super();
    }
    public EDDischargeSummarySchedule (Integer id, int ver, Boolean includeRecord)
    {
    	super(id, ver, includeRecord);
    }
	public Class getRealDomainClass()
	{
		return ims.core.configuration.domain.objects.EDDischargeSummarySchedule.class;
	}
	...
	public static EDDischargeSummarySchedule getEDDischargeSummaryScheduleFromConfiguredJobDetails(ims.domain.ILightweightDomainFactory factory, Integer id)
	{
		java.util.List l = factory.find("from EDDischargeSummarySchedule c where c.configuredJobDetails.id = " + id);
		if (l == null || l.size() == 0) return null;
		if (l.size() > 1) throw new ims.domain.exceptions.DomainRuntimeException("Non-unique get call. EDDischargeSummarySchedule.configuredJobDetails.id = " + id + " returned " + l.size() + " records. " );
		return (EDDischargeSummarySchedule)l.get(0);
	}
	...
	public String toXMLString(java.util.HashMap domMap)
	{
		...
		String keyClassName = "EDDischargeSummarySchedule";
	...
	private static void fillSetFromXMLString(java.util.Set set, org.dom4j.Element el, ims.domain.DomainFactory factory, java.util.HashMap domMap) throws Exception
	{
		...
		for(int i=0; i<size; i++) 
		{
			...
			EDDischargeSummarySchedule domainObject = getEDDischargeSummarySchedulefromXML(itemEl, factory, domMap);
	...
	public static EDDischargeSummarySchedule getEDDischargeSummarySchedulefromXML(String xml, ims.domain.DomainFactory factory, java.util.HashMap domMap) throws Exception
	{
		org.dom4j.Document doc = new org.dom4j.io.SAXReader().read(new org.xml.sax.InputSource(xml));
		return getEDDischargeSummarySchedulefromXML(doc.getRootElement(), factory, domMap);
	}
	
	public static EDDischargeSummarySchedule getEDDischargeSummarySchedulefromXML(org.dom4j.Element el, ims.domain.DomainFactory factory, java.util.HashMap domMap) throws Exception
	{
		if (el == null)
			return null;
		
		String className = el.attributeValue("type");
		if (!EDDischargeSummarySchedule.class.getName().equals(className))
		{
			Class clz = Class.forName(className);
			if (!EDDischargeSummarySchedule.class.isAssignableFrom(clz))
				throw new Exception("Element of type = " + className + " cannot be imported using the EDDischargeSummarySchedule class");
			String shortClassName = className.substring(className.lastIndexOf(".")+1);
			String methodName = "get" + shortClassName + "fromXML";
			java.lang.reflect.Method m = clz.getMethod(methodName, new Class[]{org.dom4j.Element.class, ims.domain.DomainFactory.class, java.util.HashMap.class});
			return (EDDischargeSummarySchedule)m.invoke(null, new Object[]{el, factory, domMap});
		}

		String impVersion = el.attributeValue("classVersion");
		if(!impVersion.equals(EDDischargeSummarySchedule.CLASSVERSION))
		{
			throw new Exception("Incompatible class structure found. Cannot import instance.");
		}		
		
		EDDischargeSummarySchedule ret = null;
		int extId = Integer.parseInt(el.attributeValue("id"));
		String externalSource = el.attributeValue("source");
		ret = (EDDischargeSummarySchedule)factory.getImportedDomainObject(EDDischargeSummarySchedule.class, externalSource, extId);	
		if (ret == null)
		{
			ret = new EDDischargeSummarySchedule();
		}
		String keyClassName = "EDDischargeSummarySchedule";
		...
		if (impObj != null)
		{
			return (EDDischargeSummarySchedule)impObj.getDomainObject();
		}
		else
		{
    		impObj = new ims.configuration.ImportedObject();
	...
	public static void fillFieldsfromXML(org.dom4j.Element el, ims.domain.DomainFactory factory, EDDischargeSummarySchedule obj, java.util.HashMap domMap) throws Exception
	{
