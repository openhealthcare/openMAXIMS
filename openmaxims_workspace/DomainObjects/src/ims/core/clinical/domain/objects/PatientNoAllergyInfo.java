...
public class PatientNoAllergyInfo extends ims.domain.DomainObject implements ims.domain.SystemInformationRetainer, java.io.Serializable {
	public static final int CLASSID = 1003100085;
	private static final long serialVersionUID = 1003100085L;
    ...
    public PatientNoAllergyInfo (Integer id, int ver)
    {
    	super(id, ver);
    }
    public PatientNoAllergyInfo ()
    {
    	super();
    }
    public PatientNoAllergyInfo (Integer id, int ver, Boolean includeRecord)
    {
    	super(id, ver, includeRecord);
    }
	public Class getRealDomainClass()
	{
		return ims.core.clinical.domain.objects.PatientNoAllergyInfo.class;
	}
	...
	public static PatientNoAllergyInfo getPatientNoAllergyInfoFromPatient(ims.domain.ILightweightDomainFactory factory, Integer id)
	{
		java.util.List l = factory.find("from PatientNoAllergyInfo c where c.patient.id = " + id);
		if (l == null || l.size() == 0) return null;
		if (l.size() > 1) throw new ims.domain.exceptions.DomainRuntimeException("Non-unique get call. PatientNoAllergyInfo.patient.id = " + id + " returned " + l.size() + " records. " );
		return (PatientNoAllergyInfo)l.get(0);
	}
	...
	public String toXMLString(java.util.HashMap domMap)
	{
		...
		String keyClassName = "PatientNoAllergyInfo";
	...
	private static void fillSetFromXMLString(java.util.Set set, org.dom4j.Element el, ims.domain.DomainFactory factory, java.util.HashMap domMap) throws Exception
	{
		...
		for(int i=0; i<size; i++) 
		{
			...
			PatientNoAllergyInfo domainObject = getPatientNoAllergyInfofromXML(itemEl, factory, domMap);
	...
	public static PatientNoAllergyInfo getPatientNoAllergyInfofromXML(String xml, ims.domain.DomainFactory factory, java.util.HashMap domMap) throws Exception
	{
		org.dom4j.Document doc = new org.dom4j.io.SAXReader().read(new org.xml.sax.InputSource(xml));
		return getPatientNoAllergyInfofromXML(doc.getRootElement(), factory, domMap);
	}
	
	public static PatientNoAllergyInfo getPatientNoAllergyInfofromXML(org.dom4j.Element el, ims.domain.DomainFactory factory, java.util.HashMap domMap) throws Exception
	{
		if (el == null)
			return null;
		
		String className = el.attributeValue("type");
		if (!PatientNoAllergyInfo.class.getName().equals(className))
		{
			Class clz = Class.forName(className);
			if (!PatientNoAllergyInfo.class.isAssignableFrom(clz))
				throw new Exception("Element of type = " + className + " cannot be imported using the PatientNoAllergyInfo class");
			String shortClassName = className.substring(className.lastIndexOf(".")+1);
			String methodName = "get" + shortClassName + "fromXML";
			java.lang.reflect.Method m = clz.getMethod(methodName, new Class[]{org.dom4j.Element.class, ims.domain.DomainFactory.class, java.util.HashMap.class});
			return (PatientNoAllergyInfo)m.invoke(null, new Object[]{el, factory, domMap});
		}

		String impVersion = el.attributeValue("classVersion");
		if(!impVersion.equals(PatientNoAllergyInfo.CLASSVERSION))
		{
			throw new Exception("Incompatible class structure found. Cannot import instance.");
		}		
		
		PatientNoAllergyInfo ret = null;
		int extId = Integer.parseInt(el.attributeValue("id"));
		String externalSource = el.attributeValue("source");
		ret = (PatientNoAllergyInfo)factory.getImportedDomainObject(PatientNoAllergyInfo.class, externalSource, extId);	
		if (ret == null)
		{
			ret = new PatientNoAllergyInfo();
		}
		String keyClassName = "PatientNoAllergyInfo";
		...
		if (impObj != null)
		{
			return (PatientNoAllergyInfo)impObj.getDomainObject();
		}
		else
		{
    		impObj = new ims.configuration.ImportedObject();
	...
	public static void fillFieldsfromXML(org.dom4j.Element el, ims.domain.DomainFactory factory, PatientNoAllergyInfo obj, java.util.HashMap domMap) throws Exception
	{
