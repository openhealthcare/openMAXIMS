...
public class FitForSurgeryAssesment extends ims.domain.DomainObject implements ims.domain.SystemInformationRetainer, java.io.Serializable {
	...
	/** Fit For Surgery Comments
	  * Collection of ims.careuk.domain.objects.FitForSurgeryComment.
	  */
	...
	{
		return ims.careuk.domain.objects.FitForSurgeryAssesment.class;
	}
	...
	public String toAuditString()
	{
		...
		if (fitComments != null)
		{
			...
			while (it7.hasNext())
			{
				if (i7 > 0)
					auditStr.append(",");
				ims.careuk.domain.objects.FitForSurgeryComment obj = (ims.careuk.domain.objects.FitForSurgeryComment)it7.next();
		if (obj != null)
		{
		   if (i7 == 0)
		   {
	...
	public static void fillFieldsfromXML(org.dom4j.Element el, ims.domain.DomainFactory factory, FitForSurgeryAssesment obj, java.util.HashMap domMap) throws Exception
	{
		...
		if(fldEl != null)
		{
			...
			obj.setFitComments(ims.careuk.domain.objects.FitForSurgeryComment.fromSetXMLString(fldEl, factory, obj.getFitComments(), domMap));
		}
	...
	{	
		// Look up negative instances
            ...
            if (engine.getValue(IsFitKey) == null)
            {
                  ...
                  java.util.List<ims.careuk.domain.objects.SuitableForSurgeryAssessment> list = factory.find(hqlQuery, keys, values);
            ...
            ims.careuk.domain.objects.SuitableForSurgeryAssessment suitableForSurgery = (ims.careuk.domain.objects.SuitableForSurgeryAssessment) engine.getValue(IsFitKey);
