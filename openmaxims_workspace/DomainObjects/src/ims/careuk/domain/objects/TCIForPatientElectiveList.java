...
public class TCIForPatientElectiveList extends ims.domain.DomainObject implements ims.domain.SystemInformationRetainer, java.io.Serializable {
	...
	/** CurrentOutcome */
	private ims.careuk.domain.objects.TCIOutcomeForPatientElectiveList currentOutcome;
	/** OutcomeHistory
	  * Collection of ims.careuk.domain.objects.TCIOutcomeForPatientElectiveList.
	  */
	...
	{
		return ims.careuk.domain.objects.TCIForPatientElectiveList.class;
	}
	...
	public ims.careuk.domain.objects.TCIOutcomeForPatientElectiveList getCurrentOutcome() {
		return currentOutcome;
	}
	public void setCurrentOutcome(ims.careuk.domain.objects.TCIOutcomeForPatientElectiveList currentOutcome) {
	...
	public String toAuditString()
	{
		...
		for (i16=0; i16<outcomeHistory.size(); i16++)
		{
			if (i16 > 0)
				auditStr.append(",");
			ims.careuk.domain.objects.TCIOutcomeForPatientElectiveList obj = (ims.careuk.domain.objects.TCIOutcomeForPatientElectiveList)outcomeHistory.get(i16);
		    if (obj != null)
			{
				if (i16 == 0)
				{
	...
	public static void fillFieldsfromXML(org.dom4j.Element el, ims.domain.DomainFactory factory, TCIForPatientElectiveList obj, java.util.HashMap domMap) throws Exception
	{
		...
		{
			fldEl = fldEl.element("class");		
			obj.setCurrentOutcome(ims.careuk.domain.objects.TCIOutcomeForPatientElectiveList.getTCIOutcomeForPatientElectiveListfromXML(fldEl, factory, domMap)); 
		}
		fldEl = el.element("outcomeHistory");
		if(fldEl != null)
		{
			fldEl = fldEl.element("list");	
			obj.setOutcomeHistory(ims.careuk.domain.objects.TCIOutcomeForPatientElectiveList.fromListXMLString(fldEl, factory, obj.getOutcomeHistory(), domMap));
		}
