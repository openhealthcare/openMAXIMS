...
public class DischargeSummaryPacuAndWard extends ims.domain.DomainObject implements ims.domain.SystemInformationRetainer, java.io.Serializable {
	...
	/** Onward Referral */
	private ims.careuk.domain.objects.OnwardReferral currentOnwardReferral;
	/** Onward Referrals
	  * Collection of ims.careuk.domain.objects.OnwardReferral.
	  */
	...
	{
		return ims.careuk.domain.objects.DischargeSummaryPacuAndWard.class;
	}
	...
	public ims.careuk.domain.objects.OnwardReferral getCurrentOnwardReferral() {
		return currentOnwardReferral;
	}
	public void setCurrentOnwardReferral(ims.careuk.domain.objects.OnwardReferral currentOnwardReferral) {
	...
	public String toAuditString()
	{
		...
		if (onwardReferrals != null)
		{
			...
			while (it16.hasNext())
			{
				if (i16 > 0)
					auditStr.append(",");
				ims.careuk.domain.objects.OnwardReferral obj = (ims.careuk.domain.objects.OnwardReferral)it16.next();
		if (obj != null)
		{
		   if (i16 == 0)
		   {
	...
	public static void fillFieldsfromXML(org.dom4j.Element el, ims.domain.DomainFactory factory, DischargeSummaryPacuAndWard obj, java.util.HashMap domMap) throws Exception
	{
		...
		{
			fldEl = fldEl.element("class");		
			obj.setCurrentOnwardReferral(ims.careuk.domain.objects.OnwardReferral.getOnwardReferralfromXML(fldEl, factory, domMap)); 
		}
		fldEl = el.element("onwardReferrals");
		if(fldEl != null)
		{
			fldEl = fldEl.element("set");	
			obj.setOnwardReferrals(ims.careuk.domain.objects.OnwardReferral.fromSetXMLString(fldEl, factory, obj.getOnwardReferrals(), domMap));
		}
